...
function createRequestCard(request) {
    ...
    card.innerHTML = `
        <div class="request-info">
            ...
            <p>statut: ${request.statut_global}</p>
...
function openPDFInNewTab(dataUri) {
    // 1. Extraire la partie Base64 (tout après la virgule)
    ...
    // 4. Générer une URL pour ce Blob
    const blobUrl = URL.createObjectURL(blob);
  
    // 5. Ouvrir un nouvel onglet et y injecter un embed + titre
...
async function handleValidation(status, prestataire_id) {
    try {
...
async function loadVerifications() {
    try {
        ...
        console.error('Erreur:', error);
        alert('Une erreur est survenue lors du chargement des vérifications');
    }
}
    ...
    verifications.forEach(verification => {
        ...
        card.innerHTML = `
            <div class="request-info">
                ...
                <p>statut: ${verification.statut_global}</p>
                <p>Spécialisation: ${verification.specialisation}</p>
...
async function showRequestDetails(prestataire_id) {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`/api/verification/${prestataire_id}`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
...
function displayVerificationDetails(verification) {
    ...
    rejectBtn.onclick = () => handleValidation('rejete', verification.prestataire_id);
    acceptBtn.onclick = () => handleValidation('accepte', verification.prestataire_id);
    ...
    personalInfo.innerHTML = `
        <div class="info-section">
            ...
            <p><strong>Nom d'utilisateur:</strong> ${verification.nom_utilisateur}</p>
            <p><strong>Nom:</strong> ${verification.nom}</p>
            <p><strong>Prénom:</strong> ${verification.prenom}</p>
            <p><strong>Spécialisation:</strong> ${verification.specialisation}</p>
        </div>
        <div class="info-section">
            <h4>Coordonnées</h4>
            <p><strong>Téléphone:</strong> ${verification.telephone}</p>
            <p><strong>Email:</strong> ${verification.email}</p>
            <p><strong>Adresse:</strong> ${verification.adresse}</p>
        </div>
        <div class="info-section">
            <h4>Informations professionnelles</h4>
            <p><strong>Date de naissance:</strong> ${new Date(verification.date_naissance).toLocaleDateString()}</p>
            ...
            <p><strong>Date de création de la demande:</strong> ${new Date(verification.date_creation).toLocaleDateString()}</p>
        </div>
    `;
    ...
    <div class="document-item">
      ...
      <div class="pdf-container">
        <iframe 
          src="data:application/pdf;base64,${bufferToBase64(verification.cv_pdf)}" 
          class="pdf-iframe"></iframe>
        <div class="pdf-actions">
          <button 
            class="btn view-btn"
            onclick="openPDFInNewTab('data:application/pdf;base64,${bufferToBase64(verification.cv_pdf)}')">
            <i class="fas fa-external-link-alt"></i> Ouvrir CV
          </button>
        </div>
      </div>
    </div>
    ...
    <div class="document-item">
      ...
      <div class="pdf-container">
        <iframe 
          src="data:application/pdf;base64,${bufferToBase64(verification.diplomes)}" 
          class="pdf-iframe"></iframe>
        <div class="pdf-actions">
          <button 
            class="btn view-btn"
            onclick="openPDFInNewTab('data:application/pdf;base64,${bufferToBase64(verification.diplomes)}')">
            <i class="fas fa-external-link-alt"></i> Ouvrir Diplômes
          </button>
        </div>
      </div>
    </div>
